...
dhtmlxEventable = function(a) {
    ...
    a.eventCatcher = function(a) {
        ...
        d = function() {
            for (var d = !0,
            f = 0; f < c.length; f++) if (c[f] != null) var g = c[f].apply(a, arguments),
            d = d && g;
            return d
        };
...
if (!window.dhtmlx) window.dhtmlx = {}; (function() {
    function a(a, b) {
        var d = a.callback;
        c(!1);
    ...
    function e(a) {
        if (!m.area) m.area = document.createElement("DIV"),
...
dataProcessor.prototype = {
    ...
    sendAllData: function() {
        if (this.updatedRows.length) {
            this.messages = [];
            for (var a = !0,
            b = 0; b < this.updatedRows.length; b++) a &= this.checkBeforeUpdate(this.updatedRows[b]);
    ...
    _getAllData: function() {
        for (var a = {},
        b = !1,
        c = 0; c < this.updatedRows.length; c++) {
            var d = this.updatedRows[c]; ! this._in_progress[d] && !this.is_invalid(d) && this.callEvent("onBeforeUpdate", [d, this.getState(d)]) && (a[d] = this._getRowData(d, d + this.post_delim), b = !0, this._in_progress[d] = (new Date).valueOf())
        }
        return b ? a: null
    },
    ...
    findRow: function(a) {
        for (var b = 0,
        b = 0; b < this.updatedRows.length; b++) if (a == this.updatedRows[b]) break;
        return b
    },
    ...
    finalizeUpdate: function() {
        this._waitMode && this._waitMode--; (this.obj.mytype == "tree" || this.obj._h2) && this.updatedRows.length && this.sendData();
...
dhtmlXGridObject.prototype._init_point = function() {
    ...
    function(a, b, d) {
        if (b == "connector") {
            var h = this;
            this.clearAndLoad(c.call(this, this.xmlFileUrl, a, d),
            function() {
...
scheduler._click = {
    dhx_cal_data: function(a) {
        if (scheduler._ignore_next_click) return a.preventDefault && a.preventDefault(),
        a.cancelBubble = !0,
...
scheduler._mouse_coords = function(a) {
    ...
    if (this._table_view) {
        ...
        for (var h = 0,
        h = 1; h < this._colsS.heights.length; h++) if (this._colsS.heights[h] > b.y) break;
...
scheduler._on_mouse_down = function(a, b) {
    if (a.button != 2 && !this.config.readonly && !this._drag_mode) {
        ...
        if (this._drag_mode) {
            var d = this._locate_event(b); ! this.config["drag_" + this._drag_mode] || !this.callEvent("onBeforeDrag", [d, this._drag_mode, a]) ? this._drag_mode = this._drag_id = 0 : (this._drag_id = d, this._drag_event = scheduler._lame_clone(this.getEvent(this._drag_id) || {}))
        }
...
scheduler.setCurrentView = function(a, b) {
    if (this.callEvent("onBeforeViewChange", [this._mode, this._date, b || this._mode, a || this._date])) this.updateView(a, b),
...
scheduler._reset_scale = function() {
    if (this.templates[this._mode + "_date"]) {
        ...
        for (var m = l - this._ignores_detected,
        o = 0; o < l; o++) {
...
scheduler._process_ignores = function(a, b, c, d, e) {
    ...
    if (f) for (var g = new Date(a), h = 0; h < b; h++) f(g) && (this._ignores_detected += 1, this._ignores[h] = !0, e && b++),
    g = scheduler.date.add(g, d, c)
};
scheduler._reset_month_scale = function(a, b, c) {
    ...
    m = [], i = 0; i < f; i++) {
        ...
        for (var o = 0; o < 7; o++) {
            ...
            r = "dhx_month_head";
            o === 0 && this.config.left_border && (q += " dhx_month_body_border", r += " dhx_month_head_border"); ! this._ignores_detected || !this._ignores[o] ? (n += "<div class='" + r + "'>" + this.templates.month_day(c) + "</div>", n += "<div class='" + q + "' " + g[o] + "></div></td>") : n += "<div></div><div></div>";
            m.push(c);
    ...
    for (i = 0; i < m.length; i++) t = m[i],
    this.callEvent("onScaleAdd", [this._scales[ + t], t]);
    return c
};
scheduler.getLabel = function(a, b) {
    for (var c = this.config.lightbox.sections,
    d = 0; d < c.length; d++) if (c[d].map_to == a) for (var e = c[d].options, f = 0; f < e.length; f++) if (e[f].key == b) return e[f].label;
    return ""
};
...
scheduler.date = {
    init: function() {
        for (var a = scheduler.locale.date.month_short,
        b = scheduler.locale.date.month_short_hash = {},
    ...
    str_to_date: function(a, b) {
        for (var c = "var temp=date.match(/[a-zA-Z]+|[0-9]+/g);",
        d = a.match(/%[a-zA-Z]/g), e = 0; e < d.length; e++) switch (d[e]) {
        case "%j":
        case "%d":
            c += "set[2]=temp[" + e + "]||1;";
            break;
...
scheduler.render_view_data = function(a, b) {
    ...
    for (var c = 0,
    d = a.length; c < d; c++) this._recalculate_timed(a[c]);
...
scheduler._render_month_link = function(a) {
    for (var b = this._rendered_location,
    c = this._lame_clone(a), d = a._sday; d < a._eday; d++) {
        c._sday = d;
        c._eday = d + 1;
...
scheduler._pre_render_events = function(a, b) {
    ...
    if (this._table_view) if (b) this._colsS.heights = d;
        ...
        if (g.rows) {
            ...
            e[0] = 0;
            if (q != p) f.style.top = parseInt(f.style.top) + 2 + "px",
            s.style.overflowY = "auto",
            s.style.width = parseInt(s.style.width) - 2 + "px",
...
scheduler._pre_render_events_line = function(a, b) {
    ...
    for (var e = 0; e < a.length; e++) {
        ...
        if (this._ignores[f._sday]) a.splice(e, 1),
        e--;
        else {
            c[f._sday] || (c[f._sday] = []);
            if (!b) {
                ...
                for (var m = !1,
                o = 0; o < j.length; o++) if (l = j[o], n = this._get_event_mapped_end_date(l), n.valueOf() <= f.start_date.valueOf()) {
                    m = !0;
                    f._sorder = l._sorder;
                    j.splice(o, 1);
                    f._inner = !0;
                ...
                if (!m) if (j.length) if (j.length <= j[j.length - 1]._sorder) {
                    if (j[j.length - 1]._sorder) for (o = 0; o < j.length; o++) {
                        for (var p = !1,
                        q = 0; q < j.length; q++) if (j[q]._sorder == o) {
                            p = !0;
                            break
                        }
                        if (!p) {
            ...
            if (k < this.config.first_hour || i >= this.config.last_hour) if (d.push(f), a[e] = f = this._copy_event(f), k < this.config.first_hour && (f.start_date.setHours(this.config.first_hour), f.start_date.setMinutes(0)), i >= this.config.last_hour && (f.end_date.setMinutes(0), f.end_date.setHours(this.config.last_hour)), f.start_date > f.end_date || k == this.config.last_hour) a.splice(e, 1),
            e--
        }
    }
    if (!b) {
...
scheduler._pre_render_events_table = function(a, b) {
    ...
    k = 0; k < a.length; k++) {
        ...
        n = f || i.start_date,
        m = i.end_date;
        if (n < this._min_date) l.first_chunk = !1,
        n = this._min_date;
        if (m > this._max_date) l.last_chunk = !1,
        m = this._max_date;
        ...
        if (!this._ignores[i._sday] || !i._timed) {
            ...
            i._first_chunk = l.first_chunk,
            i._last_chunk = l.last_chunk;
            else {
...
scheduler._load = function(a, b) {
    if (a = a || this._load_url) {
        ...
        if (this._load_mode) {
            for (var d = this.templates.load_format,
            b = this.date[this._load_mode + "_start"](new Date(b.valueOf())); b > this._min_date;) b = this.date.add(b, -1, this._load_mode);
            c = b;
            for (var e = !0; c < this._max_date;) c = this.date.add(c, 1, this._load_mode),
            this._loaded[d(b)] && e ? b = this.date.add(b, 1, this._load_mode) : e = !1;
            var f = c;
            do c = f,
...
scheduler.json.parse = function(a) {
    ...
    a = scheduler._temp ? scheduler._temp.data || scheduler._temp.d || scheduler._temp: [];
...
function() {
    ...
    if (a && typeof a == "object") this._obj.removeChild(a),
...
scheduler.formSection = function(a) {
    for (var b = this.config.lightbox.sections,
    c = 0; c < b.length; c++) if (b[c].name == a) break;
    var d = b[c];
...
scheduler.form_blocks = {
    ...
    time: {
        ...
        set_value: function(a, b, c, d) {
            function e(a, b, c) {
                for (var e = d._time_values,
                f = c.getHours() * 60 + c.getMinutes(), g = f, i = !1, j = 0; j < e.length; j++) {
                    var k = e[j];
                    if (k === f) {
                        i = !0;
                        break
...
scheduler._fill_lightbox = function(a, b) {
    ...
    for (var e = this.config.lightbox.sections,
    f = 0; f < e.length; f++) {
        var g = e[f],
...
scheduler.endLightbox = function(a, b) {
    ...
    this.hideCover(b);
    if (this._custom_lightbox) this._lightbox = this._temp_lightbox,
...
scheduler.getLightbox = function() {
    if (!this._lightbox) {
        ...
        for (var b = this._lightbox_template,
        c = this.config.buttons_left,
        d = 0; d < c.length; d++) b += "<div class='dhx_btn_set dhx_left_btn_set " + c[d] + "_set'><div dhx_button='1' class='" + c[d] + "'></div><div>" + scheduler.locale.labels[c[d]] + "</div></div>";
...
scheduler._init_touch_events = function() {
    ...
    if (this.config.touch) this.xy.scroll_width = 0,
...
scheduler._touch_events = function(a, b, c) {
    ...
    function e(a) {
        ...
        if (i) scheduler._on_mouse_up(b(a || event)),
    ...
    l,
    n = 0;
    dhtmlxEvent(document.body, a[0],
    function(a) {
        if (!c(a)) {
            if (i) return scheduler._on_mouse_move(b(a)),
            ...
            h = b(a);
            if (l) if (h) {
                if (g.target != h.target || Math.abs(g.pageX - h.pageX) > 5 || Math.abs(g.pageY - h.pageY) > 5) j = !0,
                clearTimeout(k)
            } else j = !0
        }
    });
    ...
    dhtmlxEvent(this._els.dhx_cal_data[0], "contextmenu",
    function(a) {
        if (l) return a && a.preventDefault && a.preventDefault(),
        (a || event).cancelBubble = !0,
        !1
    });
    dhtmlxEvent(this._els.dhx_cal_data[0], a[1],
    function(a) {
        if (!c(a)) {
            i = j = h = !1;
            l = !0;
            ...
            if (d) {
                var e = new Date;
                if (!j && !i && e - n < 250) return scheduler._click.dhx_cal_data(d),
                ...
                !1;
                n = e; ! j && !i && scheduler.config.touch_drag && (k = setTimeout(function() {
                    i = !0;
                    var a = g.target;
                    ...
                    if (scheduler._drag_mode && scheduler._drag_mode != "create") {
                        var b = -1;
                        scheduler.for_rendered(scheduler._drag_id,
                        function(a, c) {
    ...
    dhtmlxEvent(this._els.dhx_cal_data[0], a[2],
    function(a) {
        if (!c(a)) {
            i || d(g, h, 200);
            if (i) scheduler._ignore_next_click = !0;
            e(a);
            if (scheduler._block_next_stop) return scheduler._block_next_stop = !1,
            a.preventDefault && a.preventDefault(),
            a.cancelBubble = !0,
            !1
        }
