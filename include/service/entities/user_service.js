...
module.exports = function(pb) {
   ...
   /**
     * Sends a username to a user
     *
     * @method sendUsernameEmail
     * @param {Object}   user          A user object
     * @param {Function} cb            Callback function
     */
    UserService.prototype.sendUsernameEmail = function(user, cb) {
        cb = cb || util.cb;

        
        var options = {
            to: user.email,
            subject: pb.config.siteName + 'Username',
            template: 'admin/elements/send_username_email',
            replacements: {
                'first_name': user.first_name,
                'last_name': user.last_name,
    ...
    /**
     * Sends a fitbit error alert to a child account
     *
     * @method sendFitbitError_Email_child
     * @param {Object}   userID          A user's id
     * @param {Function} cb            Callback function
     */
    UserService.prototype.sendFitbitError_Email_child = function(userID, cb) {
        cb = cb || util.cb;
        var dao  = new pb.DAO();
        dao.loadById(userID, 'user', function(err, user){
            ...
            var options = {
                to: user.email,
                subject: 'Error Data From Your Fitbit Account',
                template: 'admin/elements/send_fitbit_error_email_student',
                replacements: {
                    'first_name': user.first_name,
                    'last_name': user.last_name,
                }
    ...
    /**
     * Sends a fitbit error alert to a teacher account
     *
     * @method sendFitbitError_Email_teacher
     * @param {Object}   userID          A user's id
     * @param {Function} cb            Callback function
     */
    UserService.prototype.sendFitbitError_Email_teacher = function(userID, cb) {
        cb = cb || util.cb;
        var dao  = new pb.DAO();
        var opts = {
            where: {
        ...
        dao.q("member", opts, function(err, user){
            if (util.isError(err)) {
                
            }else{
                var teacherID = user[0].parent_id;
                dao.loadById(teacherID, 'user', function(err, teacher){

                    dao.loadById(userID, 'user', function(err, child){
                        var options = {
                            to: teacher.email,
                            subject: 'Error Data From Your Child Fitbit Account',
                            template: 'admin/elements/send_fitbit_error_email_teacher',
                            replacements: {
                                'first_name': teacher.first_name,
                                'last_name': teacher.last_name,
                                'children_account' : child.username
                            }
                        };
                        var emailService = new pb.EmailService();
                        emailService.sendFromTemplate(options, cb);
      ...
      /**
     * Get all the children account based on parent username
     *
     * @method getUserByUserName
     * @param {String}   username
     * @param {Function} cb       Callback function
     */
    UserService.prototype.getUserByUserName = function(username, cb) {
        var dao   = new pb.DAO();
        var opts = {
            where: {
                username : username
     ...
     * @param {Function} cb       Callback function
     */
    UserService.prototype.getUserByUserID = function(userId, cb) {
        ...
        dao.loadById(userId, "user", function(err, data){
            if (util.isError(err)) {
                return callback(err, null);
            }
            //console.log(data);
    ...
    /**
     * Get all the children account based on parent id
     *
     * @method getChildren
     * @param {String}   user_id
     * @param {Function} cb       Callback function
     */
    UserService.prototype.getChildren = function(user_id, cb) {
        var dao   = new pb.DAO();
        var opts = {
            where: {
                parent_id: user_id+""
        ...
        dao.q("member", opts, function(err, data){
            //console.log(data);
            if (util.isError(err)) {
                return cb(err, null);
            }
    ...
    UserService.prototype.getChildrenInUser = function(user_id, cb) {
        // console.log(user_id);
        var dao   = new pb.DAO();
        var opts = {
            where: {
        ...
        dao.q("member", opts, function(err, members){
            ...
            dao.q("user", opts_children, function(err1, children){
    ...
    UserService.prototype.getRegisterChildrenInUser = function(user_id, cb) {
        // console.log(user_id);
        var dao   = new pb.DAO();
        var opts = {
            where: {
    ...
    /** add here
     * Attempts to load and parse one json file for a plugin.
     * @static
     * @method loadJsonFile
     * @param {String} filePath The absolute path to the json file
     * @param {Function} cb A callback that provides two parameters: cb(error, jsonObject)
     */
    UserService.prototype.loadJSONFile = function(filePath, cb) {
     ...
     * @param {Function} cb       Callback function
     */
    UserService.prototype.getChildrenByID = function(user_id, cb) {
        var dao   = new pb.DAO();
        var opts = {
            where: {
                parent_id : user_id+""
        ...
        var coc = [ { username: 'ruibigou1@gmail.com' },
                    { username: 'ruigou@gmail.com' } ];
        ...
        dao.q("member", opts, function(err, data){
            ...
            for (var i = data.length - 1; i >= 0; i--) {
                childrenID[i] = data[i].child_id;
            };

            // // console.log(childrenID);
            // // console.log(pb.DAO.getIdInWhere(childrenID));
            var option = {
                select: {
                    username : 1
                },
                where :pb.DAO.getIdInWhere(childrenID)
            };
            var childUserName = [];
            dao.q("user", option, function(err, users){
                //console.log(users);
     ...
     * @param {Function} cb            Callback function
     */
    UserService.prototype.sendRegisterationEmail = function(user, cb) {
        cb = cb || util.cb;

        
        var options = {
            to: user.email,
            subject: pb.config.siteName + ' Registeration Successfully Confirmation ',
            template: 'admin/elements/send_registeration_email',
            replacements: {
                'first_name': user.first_name,
                'last_name': user.last_name,
                'username': user.username
    ...
    //add here true
    /**
     * Sends a accept email to a parent of a friend
     *
     * @method sendPasswordResetEmail
     * @param {Object}   friend          A friend object
     * @param {Function} cb            Callback function
     */
    UserService.prototype.sendInvitationEmail = function(friend, cb) {
        cb = cb || util.cb;
        // console.log("cao" + friend.sender_id+"ri"+friend.sender_email);
        var acceptUrl = pb.UrlService.createSystemUrl('/actions/user/accept_invitation')
        +  util.format('?sender_id=%s&receiver_id=%s&sender_email=%s&receiver_email=%s&sender_username=%s&receiver_username=%s&parentEmailId=%s&parent_username=%s', 
            encodeURIComponent(friend.sender_id), encodeURIComponent(friend.receiver_id),
            encodeURIComponent(friend.sender_email), encodeURIComponent(friend.receiver_email),
            encodeURIComponent(friend.sender_username), encodeURIComponent(friend.receiver_username),
            encodeURIComponent(friend.parentEmailId),encodeURIComponent(friend.parent_username)
            );
        var rejectUrl = pb.UrlService.createSystemUrl('/actions/user/reject_invitation')
        +  util.format('?sender_id=%s&receiver_id=%s&sender_email=%s&receiver_email=%s&sender_username=%s&receiver_username=%s&parentEmailId=%s&parent_username=%s', 
            encodeURIComponent(friend.sender_id), encodeURIComponent(friend.receiver_id),
            encodeURIComponent(friend.sender_email), encodeURIComponent(friend.receiver_email),
            encodeURIComponent(friend.sender_username), encodeURIComponent(friend.receiver_username),
        ...
        if (parseInt(friend.receiver_age) > 13){
            var options = {
            to: friend.receiver_email,
            subject: pb.config.siteName + ' Friend Invitation',
            template: 'user/elements/invitation_email',
            replacements: {
                'accept_url': acceptUrl,
                'reject_url': rejectUrl,
                'parent_username': friend.parent_username,
                'sender_username': friend.sender_username,
                'receiver_username' : friend.receiver_username
            }
        };
        }else {
            var options = {
            to: friend.parentEmailId,
            subject: pb.config.siteName + ' Friend Invitation',
            template: 'user/elements/invitation_email',
            replacements: {
                'accept_url': acceptUrl,
    ...
    /**
     * Sends a notification email to parent, user and friend
     *
     * @method sendPasswordResetEmail
     * @param {Object}   friend          A friend object
     * @param {Function} cb            Callback function
     */
    UserService.prototype.sendInvitationNotificationEmail = function(user, cb) {
        cb = cb || util.cb;
        
        var options = {
            to: user.email,
            subject: pb.config.siteName + ' Invitation Successful',
            template: 'user/elements/invitation_notification_email',
            replacements: {
